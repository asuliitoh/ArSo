...
public abstract class RepositorioJPA<T extends Identificable> implements RepositorioString<T> {
    ...
    public void update(T entity) throws RepositorioException, EntidadNoEncontrada {
        ...
        try {
            ...
            entity = em.merge(entity);   //Utilizado para obtener la entidad del contexto de persistencia        
            em.getTransaction().commit();
    ...
    public T getById(String id) throws EntidadNoEncontrada, RepositorioException {
        ...
        EntityManager em = EntityManagerHelper.getEntityManager();
            ...
            if (instancia == null) {
                ...
                em.refresh(instancia); //Para actualizar la entidad, evitando problemas con la caché de EclipseLink
            }
            return instancia;
    ...
    public List<T> getAll() throws RepositorioException{
        ...
        EntityManager em = EntityManagerHelper.getEntityManager();
            ...
            //Query query = em.createQuery(queryString); Devuelve una lista, pero sin especificar el tipo de las entidades (Query General)
            TypedQuery<T> query = em.createQuery(queryString, getClase()); // De esta forma, se sabe el tipo a devolver (Query Tipada) 

            query.setHint(QueryHints.REFRESH, HintValues.TRUE); //Utilizado para no ejecutar la query a la caché de EclipseLink, sino a la BD, asegurando así la correcta recuperación. 
